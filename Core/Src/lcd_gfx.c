...
void LCD_DrawChar_s(char ch, const uint16_t font[], uint16_t X, uint16_t Y, uint16_t color, uint16_t bgcolor) {
    if ((ch < 31) || (ch > 127)) return;
    ...
    /* Clear background first */
    //LCD_DrawColorBurst(bgcolor,fWidth* fHeight);
    TFT_fillRectangle(X, Y, fWidth, fHeight, bgcolor);
    //TFT_drawRectangle(X, Y, fWidth, fHeight, bgcolor);
    //LCD_DrawRectangle(X, Y, fWidth, fHeight, bgcolor);
    ...
    for (j = 0; j < fBPL/*fHeight*/; j++) {
        for (i = 0; i < fWidth; i++) {
            ...
            for(int k=0;k<8;k++)
            {
            	...
            	if(b)
            	{
            		TFT_drawPixel(X + i, Y + (j*1) + k, color);
            	}
            }
...
void LCD_DrawText_s(const char* str, const uint16_t font[], uint16_t X, uint16_t Y, uint16_t color, uint16_t bgcolor) {
    uint8_t charWidth; /* Width of character */
    uint8_t fOffset = font[0]; /* Offset of character */
    uint8_t fWidth = font[1]; /* Width of font */

    while (*str) {
        LCD_DrawChar_s(*str, font, X, Y, color, bgcolor);
